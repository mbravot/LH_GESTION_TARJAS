...
class SueldoBaseCrearScreen extends StatefulWidget {
  const SueldoBaseCrearScreen({super.key});

  @override
  State<SueldoBaseCrearScreen> createState() => _SueldoBaseCrearScreenState();
}

class _SueldoBaseCrearScreenState extends State<SueldoBaseCrearScreen> {
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Registre un nuevo sueldo base para el colaborador',
                  style: TextStyle(
  ...
  Future<void> _guardar() async {
    ...
    try {
      ...
      final success = await sueldoBaseProvider.crearSueldoBase(nuevoSueldo);

      if (mounted) {
        if (success) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Sueldo base creado correctamente'),
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Error al crear sueldo base: ${sueldoBaseProvider.error ?? 'Error desconocido'}'),
              backgroundColor: Colors.red,
            ),
          );
        }
