...
class PermisoEditarScreen extends StatefulWidget {
  final Permiso permiso;

  const PermisoEditarScreen({
    Key? key,
    required this.permiso,
  }) : super(key: key);

  @override
  State<PermisoEditarScreen> createState() => _PermisoEditarScreenState();
}

class _PermisoEditarScreenState extends State<PermisoEditarScreen> {
  ...
  Future<void> _cargarDatosIniciales() async {
    ...
    try {
      ...
      // Cargar datos del permiso existente
      _cargarDatosPermiso();
      ...
      setState(() {
        ...
        _tipoPermisoSeleccionado = tipoExiste ? widget.permiso.idTipopermiso : null;
        
        // Verificar que el estado existe en la lista
        final estadoExiste = _estadosPermiso.any((e) => e['id'].toString() == widget.permiso.idEstadopermiso);
        _estadoPermisoSeleccionado = estadoExiste ? widget.permiso.idEstadopermiso : null;
      });
  ...
  void _cargarDatosPermiso() {
    // Establecer fecha
    _fechaSeleccionada = _parseFecha(widget.permiso.fecha);

    // Establecer horas
    _horasController.text = widget.permiso.horas;
  ...
  Future<void> _guardarPermiso() async {
    ...
    try {
      ...
      final permisoProvider = Provider.of<PermisoProvider>(context, listen: false);
      final success = await permisoProvider.editarPermiso(widget.permiso.id, datos);

      if (success && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Permiso actualizado correctamente'),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error al actualizar permiso: ${permisoProvider.error}'),
  ...
  void _limpiarFormulario() {
    _cargarDatosPermiso(); // Restaurar datos originales
  }
