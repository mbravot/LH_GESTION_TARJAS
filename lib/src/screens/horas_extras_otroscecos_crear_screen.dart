...
class HorasExtrasOtrosCecosCrearScreen extends StatefulWidget {
  const HorasExtrasOtrosCecosCrearScreen({Key? key}) : super(key: key);

  @override
  State<HorasExtrasOtrosCecosCrearScreen> createState() => _HorasExtrasOtrosCecosCrearScreenState();
}

class _HorasExtrasOtrosCecosCrearScreenState extends State<HorasExtrasOtrosCecosCrearScreen> {
    ...
    try {
      ...
      setState(() {
        ...
        _cecoTipos = tiposData
            ...
            .toList();
        _cecos = []; // Se cargarán cuando se seleccione un tipo
  ...
  Future<void> _cargarCecosPorTipo(String tipoCecoId) async {
    ...
    try {
      ...
      setState(() {
        ...
        _cecoSeleccionado = null; // Reset selección
        _cecoSearchController.clear(); // Limpiar buscador
      });
    } catch (e) {
      setState(() {
  ...
  Future<void> _guardar() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Horas extras otros CECOs creadas exitosamente'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error al crear horas extras otros CECOs: $e'),
            backgroundColor: Colors.red,
          ),
        );
      }
                                      ...
                                      Container(
                                        ...
                                        child: Row(
                                          children: [
                                            ...
                                            Expanded(
                                              child: Text(
                                                _cecos.firstWhere((c) => c.id == _cecoSeleccionado).nombre,
                                                style: TextStyle(
                                                  fontWeight: FontWeight.w500,
