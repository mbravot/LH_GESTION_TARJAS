...
class TrabajadorEditarScreen extends StatefulWidget {
  final String trabajadorId;

  const TrabajadorEditarScreen({
    super.key,
    required this.trabajadorId,
  });

  @override
  State<TrabajadorEditarScreen> createState() => _TrabajadorEditarScreenState();
}

class _TrabajadorEditarScreenState extends State<TrabajadorEditarScreen> {
  ...
  Future<void> _cargarDatosIniciales() async {
    ...
    try {
      // Cargar datos del trabajador y opciones usando el endpoint de edición
      final opcionesData = await ApiService.obtenerOpcionesEditarTrabajador(widget.trabajadorId);
      
      final trabajador = opcionesData['trabajador'] as Map<String, dynamic>;
      ...
      setState(() {
        ...
        // Llenar los controladores con los datos del trabajador
        _nombreController.text = trabajador['nombre']?.toString() ?? '';
        _apellidoPaternoController.text = trabajador['apellido_paterno']?.toString() ?? '';
        _apellidoMaternoController.text = trabajador['apellido_materno']?.toString() ?? '';
        _rutController.text = trabajador['rut']?.toString() ?? '';
        _codigoVerificadorController.text = trabajador['codigo_verificador']?.toString() ?? '';
        
        // Establecer valores seleccionados
        _contratistaSeleccionado = trabajador['id_contratista']?.toString();
        _porcentajeSeleccionado = trabajador['id_porcentaje']?.toString();
        _estadoSeleccionado = trabajador['id_estado']?.toString();
      });
    } catch (e) {
      setState(() {
        _error = e.toString();
  ...
  Future<void> _guardarTrabajador() async {
    ...
    try {
      ...
      final success = await trabajadorProvider.editarTrabajador(widget.trabajadorId, trabajadorData);
      ...
      if (success) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Error al actualizar trabajador: ${trabajadorProvider.error}'),
              backgroundColor: Colors.red,
            ),
          );
        }
  ...
  void _limpiarFormulario() {
    _cargarDatosIniciales(); // Recargar datos originales
  }
