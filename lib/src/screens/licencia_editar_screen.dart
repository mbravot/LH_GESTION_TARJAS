...
class LicenciaEditarScreen extends StatefulWidget {
  final Licencia licencia;
  
  const LicenciaEditarScreen({
    super.key,
    required this.licencia,
  });

  @override
  State<LicenciaEditarScreen> createState() => _LicenciaEditarScreenState();
}

class _LicenciaEditarScreenState extends State<LicenciaEditarScreen> {
  ...
  Future<void> _cargarDatosIniciales() async {
    ...
    try {
      ...
      setState(() {
        ...
        _colaboradorSeleccionado = widget.licencia.idColaborador;
        
        // Parsear las fechas
        try {
          _fechaInicio = DateTime.parse(widget.licencia.fechaInicio);
        } catch (e) {
          _fechaInicio = null;
        }
        
        try {
          _fechaFin = DateTime.parse(widget.licencia.fechaFin);
        } catch (e) {
          _fechaFin = null;
        }
      });
  ...
  Future<void> _guardarLicencia() async {
    ...
    try {
      ...
      final success = await licenciaProvider.editarLicencia(widget.licencia.id, licenciaData);

      if (success) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Licencia médica actualizada correctamente'),
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Error al actualizar licencia médica: ${licenciaProvider.error}'),
              backgroundColor: Colors.red,
            ),
          );
        }
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                _buildInfoRow('Colaborador', widget.licencia.nombreCompletoColaborador),
                                _buildInfoRow('Período', widget.licencia.periodoFormateado),
                                _buildInfoRow('Duración', '${widget.licencia.duracionDias} días'),
                                _buildInfoRow('Estado', widget.licencia.estado),
                              ],
                            ),
                          ),
                        ),
                                  ...
                                  Container(
                                    ...
                                    child: Row(
                                      children: [
                                        ...
                                        Expanded(
                                          child: Text(
                                            'Nueva duración: ${_calcularDuracionDias()} días',
                                            style: TextStyle(
                                              color: AppTheme.primaryColor,
