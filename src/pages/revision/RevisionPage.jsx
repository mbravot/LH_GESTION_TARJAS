...
import { getActividadesBySucursal } from '@/api/revision'
...
// Map API estado IDs to status keys
const ESTADO_MAP = {
  '1': 'enviada',
  '2': 'aprobada_jefe',
  '3': 'aprobada',
...
const RevisionPage = () => {
  ...
  const [filtroTipoRend, setFiltroTipoRend] = useState('todos')

  const { data: actividades = [], isLoading, isError, error } = useQuery({
    queryKey: ['revision', 'actividades', idSucursal],
  ...
  const filtered = useMemo(() => {
    ...
    if (filtroTipoRend !== 'todos') {
      result = result.filter((a) => a.id_tiporendimiento === filtroTipoRend)
    }

    return result
  }, [actividades, search, filtroContratista, filtroTipoRend])

  const clearFilters = () => {
    setSearch('')
    setFiltroContratista('todos')
    setFiltroTipoRend('todos')
  }

  const hasActiveFilters =
    search || filtroContratista !== 'todos' || filtroTipoRend !== 'todos'

  return (
    <div className="space-y-6">
      <PageHeader
        title="Revision de Tarjas"
        description={`${filtered.length} actividades encontradas`}
      />
      ...
      {showFilters && (
        <div className="flex flex-wrap gap-3 p-4 bg-card rounded-lg border border-border">
          ...
          <div className="space-y-1">
            <label className="text-xs font-medium text-text-secondary">Tipo Rendimiento</label>
            <Select value={filtroTipoRend} onValueChange={setFiltroTipoRend}>
              <SelectTrigger className="w-48">
                <SelectValue placeholder="Todos" />
              </SelectTrigger>
              <SelectContent>
                ...
                <SelectItem value="2">Grupal</SelectItem>
            ...
            {isLoading ? (
              ...
              filtered.map((act) => (
                <TableRow
                  ...
                  onClick={() => navigate(`/revision/${act.id}`, { state: { actividad: act } })}
                >
